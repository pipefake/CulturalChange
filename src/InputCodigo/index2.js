  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      if (!userData.student && !userData.visitor) {
        throw new Error(
          "Debes seleccionar al menos una opción entre Estudiante y Visitante."
        );
      }
      if (userData.student && userData.visitor) {
        throw new Error(
          "No puedes seleccionar ambas opciones al mismo tiempo."
        );
      }
  ...
  const handleIdentificationInputChange = (event) => {
    ...
    if (value === "") {
      ...
      // Si el valor no es un número entero o contiene una letra, entonces se muestra el error correspondiente
        ...
        setErrorIdentification(
          "Debe ser un número positivo de hasta 10 dígitos."
        );
      } else {
        setErrorIdentification("");
      }
